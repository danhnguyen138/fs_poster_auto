...
class YoutubeCommunity
{
	...
	public function __construct ( $channelId, array $cookies, $proxy = NULL )
	{
        $this->setCookies($cookies);
		$this->proxy   = $proxy;

        $this->rotateCookies($channelId);
    ...
    {
        if(!isset($cookies['LOGIN_INFO'], $cookies['__Secure-3PAPISID'], $cookies['__Secure-3PSID'], $cookies['__Secure-3PSIDTS'])){
            throw new Exception(fsp__('Please reauthorize Youtube Community account'));
        }

        $cook = [
            [
                "Name"     => "LOGIN_INFO",
                "Value"    => $cookies['LOGIN_INFO'],
                "Domain"   => ".youtube.com",
                "Path"     => "/",
                "Max-Age"  => NULL,
                "Expires"  => NULL,
            ...
            [
                ...
                "Value"    => $cookies['__Secure-3PAPISID'],
                "Domain"   => ".youtube.com",
                "Path"     => "/",
                "Max-Age"  => NULL,
                "Expires"  => NULL,
            ...
            [
                ...
                "Value"    => $cookies['__Secure-3PSIDTS'],
                "Domain"   => ".youtube.com",
                "Path"     => "/",
                "Max-Age"  => NULL,
                "Expires"  => NULL,
    ...
    public function rotateCookies($channelId)
    {
        if(empty($channelId))
        {
            return;
        ...
        $lastRotate = Helper::getOption('youtube_cookies_rotated_' . $channelId, 0);

        if(time() - $lastRotate < 300)
        {
            return;
        }

        Helper::setOption('youtube_cookies_rotated_' . $channelId, time());
        ...
        try{
            ...
            if(!empty($setCookies))
            {
                ...
                $channel = DB::fetch('accounts', $channelId);

                $cookies = json_decode($channel['options'], true);
                $cookies['__Secure-3PSIDTS'] = $psidts;
	...
	public function post ( $message, $type, $images )
	{
		...
		if ( ! $error = $this->init() )
		{
			...
			if ( $backstage[ "status" ] )
			{
				...
				$time     = time();
				$apiSid   = $this->cookieJar->getCookieByName( "__Secure-3PAPISID" )->getValue();
