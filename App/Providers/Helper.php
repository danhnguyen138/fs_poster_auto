...
class Helper
{
	...
	public static function scheduleFilters ( $schedule_info )
	{
		...
		{
			$get_taxs = DB::DB()->get_col( DB::DB()->prepare( "SELECT `term_taxonomy_id` FROM `" . DB::WPtable( 'term_taxonomy', TRUE ) . '` WHERE `term_id` IN (' . implode( ',', array_fill(0, count($categories_arr), '%s') ) . ')', ...$categories_arr ), 0 );
