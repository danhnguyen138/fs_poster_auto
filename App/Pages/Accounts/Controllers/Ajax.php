...
trait Ajax
{
    ...
    public function add_instagram_account ()
    {
        ...
        $insta = new InstagramLoginPassMethod( [ 'username' => $username, 'password' => $password ], $proxy );
        $result  = $insta->login();
    ...
    public function do_instagram_challenge()
    {
        ...
        $insta = new InstagramLoginPassMethod( $options, $proxy );
        $result  = $insta->doTwoFactorAuth( $options[ 'two_factor_identifier' ], $code, $options['verification_method'] );
    ...
    public function add_threads_account ()
    {
        ...
        if( isset( $result['data']['needs_challenge'] ) && $result['data']['needs_challenge'] )
        {
            Helper::response( true, [ 'options' => $result['data']['options'] ] );
        }
	...
	public function add_youtube_community_account ()
	{
        ...
        $secure3PSidTS = Request::post( 'psidts', '', 'string' );
        $proxy         = Request::post( 'proxy', '', 'string' );

		if ( empty( $loginInfo ) || empty( $secure3ApiSid ) || empty( $secure3PSid ) || empty( $secure3PSidTS ) )
		{
		...
		$response = ( new YoutubeCommunity( null, $cookies, $proxy ) )->channel();
