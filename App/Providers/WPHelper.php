...
trait WPHelper
{
	...
	public static function getPostTerms ( $postInf, $tax_name = NULL, $addSharp = TRUE, $asArray = TRUE, $separator = ' ', $replaceWhiteSpaces = true )
	{
		if ( empty( $tax_name ) )
		{
		...
		$replaceWhitespacesWith = Helper::getOption( 'replace_whitespaces_with_underscore', '0' ) == 1 ? '_' : '';
		$terms_arr          = [];
		...
		foreach ( $terms as $termInf )
		{
            ...
            if($replaceWhiteSpaces){
                $formatted_tag = preg_replace( [ '/\s+/', '/&+/', '/-+/' ], $replaceWhitespacesWith, $formatted_tag );
            }
