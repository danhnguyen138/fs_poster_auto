...
class Listener
{
	...
	public static function sharePost( $result, $postData )
	{
        ...
        $yAccount = DB::fetch('accounts', [
            'driver' => 'youtube_community',
            'profile_id' => $postData->getAccoundId()
        ]);

		//todo: status burdan string kimi qayıtsın, ya da digerlerinden de boolean.
		$res = ( new YoutubeCommunity( $yAccount['id'], $cookies, $postData->getProxy() ) )->post( $postData->message, $postData->sendType, $media );
